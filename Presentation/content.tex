...
episode $\gets 0$\;
\While{episode < $N$}{
  выбираем задачу $t \sim T$\;
  env.set\_task(t)\;
  batch = rollaut(agent, env)\;
  agent.train(batch)\;
  env.update\_reward(episode)\;
}
\end{algorithm}
\end{columns}
\end{minipage}
...
\vspace{5pt}
\setcounter{footnote}{0} 
Связанные работы (нет многозадачности)\footnote[frame]{Hwangbo, J. et. all, “Learning agile and dynamic motor skills for legged robots”, Science Robotics, vol. 4, Jan. 2019.}\footnote[frame]{Lee, J. et. all “Learning quadrupedal locomotion over challenging terrain,” Science Robotics, vol. 5, no. 47, 2020.}
\end{minipage}
...
\KwData{condition, agent, <skills>}
s $\gets$ env.reset()\;
\While{not done}{
  \eIf{condition(s)}{
    a = agent.act(s)\;
  }{
    a = best\_fit(skills, s)
  }
